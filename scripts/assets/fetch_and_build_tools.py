...
from scripts.database.add_tool import add_tools

# for imports to work needs to be run from the root directory
# run command: python -m scripts.assets.fetch_and_build_tools
# --text to input a single string --file to specify a file of image slugs and names
# to define a icon specify the name then the simple icons slug separated by a comma with more the next icon entered on the next line.
# {name},{slug}

# config
ICON_DIR = Path(__file__).parent.parent.parent / "backend/app/static/media/icons/svg/tools"
ICON_DIR.mkdir(parents=True, exist_ok=True)
ICON_LINK_TEMPLATE = "/static/media/icons/svg/tools"

ICON_URL = "https://raw.githubusercontent.com/simple-icons/simple-icons/develop/icons/{slug}.svg"


# icon downloader, takes a slug and return the local file path or None if not found
